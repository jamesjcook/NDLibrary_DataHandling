...
our %cmd_flags = (
    ...
    #'file'=> '-pt',
    'file'=> '-p',
    # This debug doesnt use the "requsted" option value, it starts at 20, and goes up by 10 foreach nested lib.
    'lib' => '-d '.($debug_val+10),
    'rm'  => '-fr'
    );
...
# I keep reducing template members in favor of coding the information into that individual lib.conf
...
sub Main
{
    ...
    if ($can_dump){
        Data::Dump::dump(["All work from $source_path",$work]);
    }
...
sub path_collapse
{
    # The path collapse algorithm works by starting on the left most dir
    # element and adding each element in order.
    # When a double dot is found, instead of adding, it removes the previous
    # trailing element.
    my($path)=@_;
    ...
    foreach (@p_parts) {
        if ($_ !~ /[.]{2}/ ) {
            # path element is NOT a double dot, append
            print($indent."$_ (add) \n") if ($debug_val>40);
            push(@outpath,$_);
            ...
            # path element is a double dot, remove one dir element.
            my $rem=pop(@outpath);
                    ...
                    if ( $in_ts > $out_ts || -d $f_inpath  ) { # if its newer, we'll want to transfer that. #
                        ...
                        if ( -f $f_inpath){
                            ...
                            if ( -f $f_inpath.'/lib.conf' ) {
                                ...
                                # dirs are forcing a trailing slash on name. Important for rsync operations. Complicating feedback.
                                $work_list->{$_.'/'}='dir';
                            }
                        }
                        if (exists $lib_template{$_}) {
                            ...
                            # Current dir implemntation cant be supported by lib templatess, so we'll sleep and die if we find one.
                            if( exists $work_list->{$_.'/'} ) {
                                carp("Sorry, error with directory type overriding, we dont handle that right");
                                sleep_with_countdown(120);
                                confess("EMERGENCY TERMINATION");
                            }
                        }
                        if ( defined $work_list->{$_} || defined $work_list->{$_.'/'} ) {
                            # complicated one liner for either $_ or $_/
                            my $k= defined $work_list->{$_} ? $_ : $_.'/';
                            print("Add $f_inpath ( $k ) <- ".$work_list->{$k}."\n");
                        } else {
...
sub do_work
{
    ...
    foreach (keys %$work ){
        ...
        my $cmd_flags=$cmd_flags{$work->{$_}}; # work vall will be either dir|file
        ...
        #push(@{$lib_stacks->{$lib_path}},$conf_inpath);
        if ( $work->{$_} =~ /lib/ ) {
            my $feedback_ls=$lib_stacks->{$_};
            if (scalar @{$lib_stacks->{$_}}>0 ) {
                $lc='-c '.join(",",@{$lib_stacks->{$_}} );
            } else {
                $feedback_ls="Lib $_ libstack not defined.";
            }
            if ($can_dump){
                Data::Dump::dump(["Current lib stacks for $_",$feedback_ls]);
            }
            $in_p="\'".$in_dir."\'";
            $outp="\'".$out_dir."\' \'$_\'";
        } else {
            printd(45,"Non lib update $_\n");
        }
        $cmd= "$update $lc $cmd_flags $in_p $outp";
        printd(45,$cmd."\n");
#        if ($work->{$_} =~ /rm/ ) {
#            $cmd= "$update_cmd{$work->{$_}} $cmd_flags $out_p";
#        }
